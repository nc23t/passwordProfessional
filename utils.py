...
def length(user_password):
    if len(user_password) <= 6:
        print("Your password is only", len(user_password),
              "characters long, add a lot more!")
        ptsL = 2
        print('Your score for length is', ptsL, 'points')
        return ptsL
    elif 6 < len(user_password) <= 10:
        print('Your password length is', len(user_password),
              'characters, you could add more')
        ptsL = 3
        print('Your score for length is', ptsL, 'points')
        return ptsL
    elif 11 <= len(user_password) <= 16:
        print('Your password is', len(user_password),
              'characters, add just a little more!')
        ptsL = 5
        print('Your score for length is', ptsL, 'points')
        return ptsL
    elif 17 <= len(user_password) < 20:
        print('Your password is', len(user_password),
              'characters, you can make it to 20!')
        ptsL = 7
        print('Your score for length is', ptsL, 'points')
        return ptsL
    elif len(user_password) == 20:
        print('Your password is', len(user_password),
              'characters long, perfect length!')
        ptsL = 10
        print('Your score for length is', ptsL, 'points')
        return ptsL
    elif len(user_password) > 20:
        print('Your password is', len(user_password),
              "characters, this too long!")
        ptsL = 7
        print('Your score for length is', ptsL, 'points')
        return ptsL
